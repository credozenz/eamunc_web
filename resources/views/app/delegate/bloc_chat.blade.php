    ...
    <div class="container-fluid dasboard">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                @if (!empty($blocs_members))
                    @foreach ($blocs_members as $key => $each)
                        @if (!empty($each->avatar))
                            <img src="{{ asset('uploads/' . $each->avatar) ?? '' }}" alt="{{ $each->name ?? '' }}"
                                title="{{ ucfirst($each->name) ?? '' }}" class="avatar rounded-circle d-inline-block"
                                style="width: 40px; height: 40px;">
                        @else
                            <img src="{{ asset('assets/img/avatar.svg') }}" alt="{{ $each->name ?? '' }}"
                                title="{{ ucfirst($each->name) ?? '' }}" class="avatar rounded-circle d-inline-block"
                                style="width: 40px; height: 40px;">
                        @endif
                    @endforeach
                ...
                <input type="hidden" id="committee_bloc_id" value="{{ $committee_bloc->id }}">
            </div>

            <div class="col-md-3 "></div>

            <div class="col-md-3 ">
                <h5 class="text-primary mb-3 d-inline-block ps-2 " style="line-height: 40px;">
                    {{ ucfirst($committee->name) ?? '' }}</h5>
            </div>
                                        ...
                                        <div class="chat">
                                            <div class="chat-avatar">
                                                <a class="avatar avatar-online" data-toggle="tooltip" data-placement="right"
                                                    title="" data-original-title="{{ $chat->user_name ?? '' }}">
                                                    @if (!empty($chat->avatar))
                                                        <img src="{{ asset('uploads/' . $chat->avatar) ?? '' }}"
                                                            alt="{{ $chat->user_name ?? '' }}"
                                                            title="{{ ucfirst($chat->user_name) ?? '' }}">
                                                    @else
                                            ...
                                            <div class="chat-body">
                                                <div class="chat-content">
                                                    <small
                                                        style="font-size: 10px">{{ ucfirst($chat->cntry_name) ?? '' }}</small>
                                                    @if ($chat->type == 'text')
                                                        <p>{{ $chat->message ?? '' }}</p>
                                                    @elseif($chat->type == 'image')
                                                        <img class="rounded-md img-preview"
                                                            src="{{ asset('uploads/' . $chat->message) }}"
                                                            style="width: 250px;">
                                                    ...
                                                    <time
                                                        class="chat-time">{{ date('d-m-Y g:i a', strtotime($chat->created_at)) ?? '' }}</time>

                                                    @if ($chat->user_id == $member->user_id)
                                                        <form method="post"
                                                            action="{{ url('app/delegate_chat_dlt', $chat->id) }}"
                                                            class="col-md-12 mt-4" enctype="multipart/form-data">
                                                            @csrf
                                                            <hr>
                                                            <span class="input-group-btn">
                                                                <button class="btn btn-sm btn-outline-secondary"
                                                                    title="Delete" type="submit"
                                                                    style="font-size: 10px;"><i class="fa fa-trash"
                    ...
                    @if ($block_exists == true && $committee_bloc->is_closed == 0)
                        <div class="panel-footer">
                            <form method="post" action="{{ url('app/delegate_chat_store', $id) }}" class="col-md-12"
                                ...
                                @csrf
                                <div class="input-group chat_frm_area">
                                    ...
                                    <label id="crs"
                                        style="display:none; padding-left: 58px;color: red;position: absolute;">x</label>
                                    <input id="file-upload" name='file' type="file" style="display:none;">
                                    <span class="input-group-btn">
                                        <button class="btn btn-outline-primary" type="submit"
                                            style="border-radius:0px  ;"><i class="fa fa-paper-plane"
                                                aria-hidden="true"></i></button>
                                    </span>
                                </div>
                                <span class="d-none" id="chat_blk_area"></span>
                            </form>
                        </div>
 ...
 <p id="counter">0</p> --}}
    ...
    <script>
        ...
        messageInput.addEventListener('input', function() {
            ...
            if (check_time > 0) {
                $(".chat_frm_area").addClass('d-none');
                $("#chat_blk_area").removeClass('d-none');
                setCoolOffTime(check_time);
            } else {
                $(".chat_frm_area").removeClass('d-none');
                $("#chat_blk_area").addClass('d-none');
            }

            $('body').on('submit', '#chat_frm', function(e) {
                var check_time = localStorage.getItem("counter_" + $("#committee_bloc_id").val());
                if (check_time > 0) {
                    e.preventDefault();
                    return false;
            ...
            function setCoolOffTime(check_time) {
                var counter = check_time > 0 ? check_time : 0;
                var interval = setInterval(function() {
                    counter--;
                    localStorage.setItem("counter_" + $("#committee_bloc_id").val(), counter);
                    $("#chat_blk_area").text('You will be able to send a message in ' + counter + ' sec');
                    if (counter == 0) {
                        $(".chat_frm_area").removeClass('d-none');
                        $("#chat_blk_area").addClass('d-none');
                        $("#chat_blk_area").text('You will be able to send a message in ' + check_time +
                            ' sec');
