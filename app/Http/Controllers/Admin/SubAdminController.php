...
class SubAdminController extends Controller
{
    ...
    {
        View::share('routeGroup','sub_admins');
    }
  
    public function index(Request $request)
    {   
        $query = User::where('deleted_at', null)->where('role', 9);
        if($request->q){
            $search_text = $request->q;
            $query->whereRaw("(name like '%$search_text%' OR email like '%$search_text%')");
        }
        $data=$query->orderBy('id', 'DESC')
                    ->paginate(10); 
        return view('admin/sub_admins/index', compact('data','request'));
    }
        ...
        $password = $request->input('password');
                ...
                if(!empty($useremail->count())){
                    ...
                    if ($request->hasFile('image')) {
                        ...
                        $fileName   =  time().'_sa_'.str_random(5).'_'.rand(1111,9999). '.' . $image->getClientOriginalExtension();
            ...
            if(isset($user->id)){
                Session::flash('success', 'Sub Admin Added successfully Completed!');
    ...
    public function show($id)
    {
        ...
        return view('admin/sub_admins/show', compact('data'));
    }
    ...
    {
        $data  = User::find($id); 
        return view('admin/sub_admins/edit', compact('data'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!empty($useremail->count())){
            ...
            if(isset($user->id)){
                Session::flash('success', 'Sub Admin Updated successfully Completed!');
                return redirect('/admin/sub_admins');
            }else{
                Session::flash('error', 'Something went wrong!!');
                return  redirect()->back();
    ...
    public function destroy(Request $request,$id)
    {
        ...
        echo json_encode(['status'=>true,'message'=>'Sub Admin Deleted Successfully !']);exit();
    }
